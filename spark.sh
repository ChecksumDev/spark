...
if [ "$(id -u)" != "0" ]; then
    echo "You must be root to run this script, please use root to generate the mirrors."
    exit 1
fi
...
if [ -z "$(ping -c 1 -W 1 1.1.1.1)" ]; then
    echo "$RED> No internet connection detected, please connect to the internet and run this script again$NC"
    exit 1
fi
...
echo -e "$GREEN> Generating mirror list."
